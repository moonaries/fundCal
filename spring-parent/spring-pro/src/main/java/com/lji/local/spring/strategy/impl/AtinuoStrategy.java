...
@Service(value="atinuoStrategy")
public class AtinuoStrategy implements IStrategy<Map<String,Object>,BigDecimal> {
	...
	public BigDecimal operate(Map<String, Object> map) {
		...
		if(StringUtils.isNotEmpty(id)) {
			String sql = "SELECT ror FROM ism_fund_week_ror WHERE ror < 0 AND fund_id = #{id} "
					+ "AND date_ror >= #{stime} AND date_ror < #{etime}";
			List<BigDecimal> rors = dceDaoEx.getRors(sql, id, ts.getStart(), ts.getEnd());
			
			String allSql = "SELECT ror FROM ism_fund_week_ror WHERE fund_id = #{id} "
					+ "AND date_ror >= #{stime} AND date_ror < #{etime}";
			List<BigDecimal> allRors = dceDaoEx.getRors(allSql, id, ts.getStart(), ts.getEnd());
			if(rors!=null && rors.size() > 9) {
				// 无风险收益率
				BigDecimal rfProfit = dceDaoEx.getLatestDepositRate(ts.getEnd());
				// 收益率平均值
				BigDecimal avgProfit = MathUtil.avg(allRors);
				...
				return (avgProfit.subtract(rfProfit.divide(new BigDecimal(52), Commons.ACCURACY, RoundingMode.HALF_UP)).abs())
